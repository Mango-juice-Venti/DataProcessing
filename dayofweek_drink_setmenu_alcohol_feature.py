...
# === 전처리: 날짜/메뉴명===
df['영업일자'] = pd.to_datetime(df['영업일자'])
df['seasonal_index'] = df['영업일자'].dt.quarter #1~4
...
#1. 영업장+분기별 평균 대비
df['avg_store_quarter'] = df.groupby(['영업장명', 'seasonal_index'])['매출수량'].transform('mean')
df['is_popular_menu_quarter'] = (df['매출수량'] > df['avg_store_quarter']).astype(int)

#2. 메뉴별 평균 대비
df['avg_menu'] = df.groupby('메뉴명')['매출수량'].transform('mean')
df['is_popular_menu_menu'] = (df['매출수량']> df['avg_menu']).astype(int)
...
#평균 컬럼 제거하기 
df = df.drop(columns=['avg_store_quarter', 'avg_menu', 'avg_store'])
